    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-slate-700/80 backdrop-blur-sm p-2 border border-slate-600 rounded-md shadow-lg">
          ...
          <p className="text-sm text-cyan-300 font-bold">{`WPM: ${payload[0].value}`}</p>
        </div>
      );
    }
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <YAxis 
            ...
            tickLine={false} 
            axisLine={false}
            label={{ value: 'WPM', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 12 }}
        />
        <Tooltip content={<CustomTooltip />} />
        <Area type="monotone" dataKey="wpm" stroke="#22d3ee" strokeWidth={2} fill="url(#colorWpm)" />
      </AreaChart>
    </ResponsiveContainer>
  );
};
